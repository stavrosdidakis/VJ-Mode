...
vec4 freeze(vec4 myInput, vec4 previousmix, vec4 amount)
{
	vec4 branch1 = (one - previousmix);
	vec4 result = one - (branch1 * branch1) / myInput;
...
{ 
	...
	vec4 mix1 = freeze(input1, input0, amount * 2.0);
	vec4 mix2 = freeze(input0, input1, (1.0 - amount) * 2.0);
